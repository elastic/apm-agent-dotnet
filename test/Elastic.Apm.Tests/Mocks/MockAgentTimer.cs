...
namespace Elastic.Apm.Tests.Mocks
{
	...
	internal class MockAgentTimer : IAgentTimerForTesting
	{
		...
		private async Task DelayAsyncImpl(AgentTimeInstant until, CancellationToken cancellationToken = default)
		{
			...
			{
				_logger.Trace()?.Log($"Delay item already reached its trigger time. When to trigger: {until}. now: {now}.");
				return;
			}

			_logger.Trace()?.Log($"Added delay item. When to trigger: {until}. Delay item ID: {delayItemId.Value}");
