...
public class FilterTests
{
	...
	public async Task RenameTransactionNameAndTypeIn2Filters() =>
		await RegisterFilterRunCodeAndAssert(
			...
			agent => { agent.Tracer.CaptureTransaction("Test123", "TestTransaction", _ => { Thread.Sleep(10); }); },
			(transactions, _, _) =>
			{
	...
	{
		var mockConfig = new MockConfiguration(maxBatchEventCount: "1", flushInterval: "0");
		...
		timer.Elapsed += (_, _) => { taskCompletionSource.SetCanceled(); };
		timer.Start();
		...
		var handler = RegisterHandlerAndAssert((transactions, spans, errors) =>
		{
			try
			{
				if (assert(transactions, spans, errors))
