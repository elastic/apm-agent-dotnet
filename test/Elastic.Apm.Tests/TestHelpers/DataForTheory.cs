...
namespace Elastic.Apm.Tests.TestHelpers
{
	...
	{
		internal void Add(T p) => AddRow(p);
	}

	public class DataForTheory<T1, T2> : DataForTheory
	{
		internal void Add(T1 p1, T2 p2) => AddRow(p1, p2);
	}

	public class DataForTheory<T1, T2, T3> : DataForTheory
	{
		internal void Add(T1 p1, T2 p2, T3 p3) => AddRow(p1, p2, p3);
	}


	public class DataForTheory<T1, T2, T3, T4> : DataForTheory
	{
		internal void Add(T1 p1, T2 p2, T3 p3, T4 p4) => AddRow(p1, p2, p3, p4);
	}

	public class DataForTheory<T1, T2, T3, T4, T5> : DataForTheory
	{
		internal void Add(T1 p1, T2 p2, T3 p3, T4 p4, T5 p5) => AddRow(p1, p2, p3, p4, p5);
	}

	public class DataForTheoryTests
	{
		// ReSharper disable once MemberCanBePrivate.Global
		public static DataForTheory<int> Test1ArgDataForTheorySampleData => new DataForTheory<int> { 137 * 2, 137 * 3, 137 * 4 };

		// ReSharper disable once MemberCanBePrivate.Global
		public static DataForTheory<int, string> Test2ArgsDataForTheorySampleData => new DataForTheory<int, string>
		{
			{ 123, "7B" }, { 987654, "F1206" }, { int.MaxValue, "7FFFFFFF" }
		};
		...
		// ReSharper disable once MemberCanBePrivate.Global
		public static DataForTheory Test5ArgsDataForTheorySampleData =>
			new DataForTheory<TypedInt<TA>, TypedInt<TB>, TypedInt<TC>, TypedInt<TD>, TypedInt<TE>>
			{
		...
		public void Test1ArgDataForTheory(int p) => (p % 137).Should().Be(0);

		[Theory]
		[MemberData(nameof(Test2ArgsDataForTheorySampleData))]
		public void Test2ArgsDataForTheory(int i, string iHex) => i.ToString("X").Should().Be(iHex);

		[Theory]
		[MemberData(nameof(Test3ArgsDataForTheorySampleData))]
		private void Test3ArgsDataForTheory(TypedInt<TA> p1, TypedInt<TB> p2, TypedInt<TC> p3) => (p1.Value + p2.Value).Should().Be(p3.Value);
		...
		private struct TypedInt<TType>
		{
			...
			internal TypedInt(int value) => Value = value;
		}
