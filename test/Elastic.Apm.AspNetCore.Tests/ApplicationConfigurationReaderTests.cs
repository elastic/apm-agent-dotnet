...
{
	/// <summary>
	/// Tests the <see cref="ApplicationConfigurationReader" /> class. It loads the JSON config files from the TestConfig folder.
	/// </summary>
	public class ApplicationConfigurationReaderTests
	{
		...
		{
			var logger = new TestLogger();
			var serverUrl = new ApplicationConfigurationReader(GetConfig($"TestConfigs{Path.DirectorySeparatorChar}appsettings_invalid.json"), logger).ServerUrls.FirstOrDefault();
