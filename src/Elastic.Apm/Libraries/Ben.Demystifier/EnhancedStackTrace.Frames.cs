...
namespace System.Diagnostics
{
	public partial class EnhancedStackTrace
	{
			...
			try
			{
				for (var i = 0; i < stackFrames.Length; i++)
				{
					...
					if (method != null && string.IsNullOrEmpty(fileName) && ilOffset >= 0)
					{
						...
						(portablePdbReader ??= new PortablePdbReader()).PopulateStackFrame(frame, method, frame.GetILOffset(), out fileName, out row,
							out column);
					}
		...
		private static bool TryResolveGeneratedName(ref MethodBase method, out Type? type, out string methodName, out string? subMethodName,
			out GeneratedNameKind kind, out int? ordinal
		)
		{
			...
			var candidateMethods =
				dt.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Instance | BindingFlags.DeclaredOnly)
			...
			var candidateConstructors =
				dt.GetConstructors(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Instance
			...
			for (var i = 0; i < MaxResolveDepth; i++)
			{
				...
				{
					candidateConstructors =
						dt.GetConstructors(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.DeclaredOnly)
							.Where(m => m.Name == matchName);
		...
		private static bool TryResolveSourceMethod(IEnumerable<MethodBase> candidateMethods, GeneratedNameKind kind, string? matchHint,
			ref MethodBase method, ref Type? type, out int? ordinal
		)
		{
			ordinal = null;
		...
		private static void GetOrdinal(MethodBase method, ref int? ordinal)
		{
			...
			if (lamdaStart > 3)
			{
				...
				var methods = method.DeclaringType?.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static
		...
		private static ResolvedParameter GetParameter(ParameterInfo parameter)
		{
			...
			{
				Prefix = prefix, Name = parameter.Name, IsDynamicType = parameter.IsDefined(typeof(DynamicAttribute), false)
			};
		}
		...
		private static ResolvedParameter GetValueTupleParameter(IList<string> tupleNames, string prefix, string? name, Type parameterType) =>
			new ValueTupleResolvedParameter(parameterType, tupleNames) { Prefix = prefix, Name = name, };
