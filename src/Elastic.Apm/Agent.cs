...
namespace Elastic.Apm
{
	public static class Agent
	{
		...
		public static AbstractAgentConfig Config
		{
			get
			{
				if (config?.Logger == null) config.Logger = CreateLogger("Config");
		...
		public static IPayloadSender PayloadSender
		{
			get
			{
				if (payloadSender == null) payloadSender = new PayloadSender();

				return payloadSender;
			}

			internal set => payloadSender = value;
		}
		...
		internal static void SetLoggerType<T>() where T : AbstractLogger, new() => loggerType = typeof(T);
	}
}
