...
#pragma warning restore 4014
		...
		private async Task WorkLoop()
		{
			...
			_logger.Context[DbgUtils.CurrentDbgContext(_dbgName)] = "Entering...";
			...
			_logger.Context[DbgUtils.CurrentDbgContext(_dbgName)] = "Signaling work loop completed event...";
		...
		public void Dispose()
		{
			_logger.Context[DbgUtils.CurrentDbgContext(_dbgName)] = "Entered";

			if (!_isEnabled)
			{
				_logger.Debug()?.Log("Disabled - nothing to dispose, exiting");
				_logger.Context[DbgUtils.CurrentDbgContext(_dbgName)] = "Exiting (disabled) ...";
				return;
			}
			...
			_disposableHelper.DoOnce(_logger, _dbgName, () =>
			{
				...
				_logger.Context[DbgUtils.CurrentDbgContext(_dbgName)] = "Posting CtsInstance.Cancel() to default (ThreadPool) TaskScheduler...";
				Task.Run(() =>
				{
					_logger.Context[DbgUtils.CurrentDbgContext(_dbgName)] = "Calling CtsInstance.Cancel()...";
					_logger.Debug()?.Log("Calling CtsInstance.Cancel()...");
					// ReSharper disable once AccessToDisposedClosure
					CtsInstance.Cancel();
					_logger.Debug()?.Log("Called CtsInstance.Cancel()");
					_logger.Context[DbgUtils.CurrentDbgContext(_dbgName)] = "Called CtsInstance.Cancel()";
				});
				_logger.Context[DbgUtils.CurrentDbgContext(_dbgName)] = "Posted CtsInstance.Cancel() to default (ThreadPool) TaskScheduler";
				_logger.Debug()?.Log("Posted CtsInstance.Cancel() to default (ThreadPool) TaskScheduler");

				_logger.Context[DbgUtils.CurrentDbgContext(_dbgName)] = "Before _loopCompleted.Wait()."
					+ $" IsCancellationRequested: {CtsInstance.Token.IsCancellationRequested}";
				_logger.Debug()
				...
				_loopCompleted.Wait();
				_logger.Context[DbgUtils.CurrentDbgContext(_dbgName)] = "After _fetchingLoopCompleted.Wait()";
				...
				_logger.Context[DbgUtils.CurrentDbgContext(_dbgName)] = "Exiting...";
			});
		}
