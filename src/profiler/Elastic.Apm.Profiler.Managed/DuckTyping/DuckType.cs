...
namespace Elastic.Apm.Profiler.Managed.DuckTyping
{
	...
	public static partial class DuckType
	{
			...
			lock (_locker)
			{
				try
				{
					...
					else
					{
						...
						typeAttributes = TypeAttributes.Public | TypeAttributes.Class | TypeAttributes.AutoClass | TypeAttributes.AnsiClass
							| TypeAttributes.BeforeFieldInit | TypeAttributes.AutoLayout | TypeAttributes.Sealed;
						interfaceTypes = new[] { typeof(IDuckType) };
					}

					// Gets the module builder
					var moduleBuilder = GetModuleBuilder(targetType,
						(targetType.IsPublic || targetType.IsNestedPublic) && (proxyDefinitionType.IsPublic || proxyDefinitionType.IsNestedPublic));
					...
					var proxyTypeName =
						$"{assembly}.{targetType.FullName.Replace(".", "_").Replace("+", "__")}.{proxyDefinitionType.FullName.Replace(".", "_").Replace("+", "__")}_{++_typeCount}";
					...
					if (proxyDefinitionType.IsValueType)
					{
						...
						return new CreateTypeResult(proxyDefinitionType, proxyType, targetType,
							CreateStructCopyMethod(moduleBuilder, proxyDefinitionType, proxyType, targetType), null);
					}
					else
					{
						...
						var proxyType = proxyTypeBuilder.CreateTypeInfo().AsType();
						return new CreateTypeResult(proxyDefinitionType, proxyType, targetType,
							GetCreateProxyInstanceDelegate(moduleBuilder, proxyDefinitionType, proxyType, targetType), null);
					}
				}
				catch (Exception ex)
				{
		...
		{
			var selectedProperties = new List<PropertyInfo>(proxyDefinitionType.IsInterface
				? proxyDefinitionType.GetProperties()
				: GetBaseProperties(proxyDefinitionType));
				...
				switch (duckAttribute.Kind)
				{
					case DuckKind.Property:
						...
						catch
						{
							...
							targetProperty = targetType.GetProperty(duckAttribute.Name, proxyProperty.PropertyType,
		...
		private static void CreatePropertiesFromStruct(TypeBuilder proxyTypeBuilder, Type proxyDefinitionType, Type targetType,
				...
				switch (duckAttribute.Kind)
				{
					case DuckKind.Property:
						...
						propertyBuilder.SetGetMethod(
							GetPropertyGetMethod(proxyTypeBuilder, targetType, proxyFieldInfo, targetProperty, instanceField));
						break;
		...
		public readonly struct CreateTypeResult
		{
			...
			internal CreateTypeResult(Type proxyTypeDefinition, Type proxyType, Type targetType, Delegate activator,
