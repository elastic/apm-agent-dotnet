...
namespace Elastic.Apm.Profiler.Managed.DuckTyping
{
	...
	internal static class ILHelpersExtensions
	{
		...
		internal static void CreateDelegateTypeFor(TypeBuilder proxyType, DynamicMethod dynamicMethod, out Type delType, out MethodInfo invokeMethod)
		{
			...
			var delegateType = modBuilder.DefineType($"{dynamicMethod.Name}Delegate_" + Guid.NewGuid().ToString("N"),
				TypeAttributes.Class | TypeAttributes.Public | TypeAttributes.Sealed | TypeAttributes.AnsiClass | TypeAttributes.AutoClass,
				typeof(MulticastDelegate));

			// Delegate .ctor
			var constructorBuilder =
				delegateType.DefineConstructor(MethodAttributes.RTSpecialName | MethodAttributes.HideBySig | MethodAttributes.Public,
		...
		internal static void WriteDynamicMethodCall(this LazyILGenerator il, DynamicMethod dynamicMethod, TypeBuilder proxyType)
		{
			...
			var fillDelegateMethodInfo = typeof(Elastic.Apm.Profiler.Managed.DuckTyping.DuckType.DelegateCache<>).MakeGenericType(delegateType)
				.GetMethod("FillDelegate", BindingFlags.NonPublic | BindingFlags.Static);
