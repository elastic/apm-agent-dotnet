...
#pragma warning restore 649
		...
		public MethodBuilder<TDelegate> WithParameters<TParam1, TParam2, TParam3, TParam4>(TParam1 param1, TParam2 param2, TParam3 param3,
			TParam4 param4
		)
		{
		...
		private MethodInfo VerifyMethodFromToken(MethodInfo methodInfo)
		{
			...
			var detailMessage =
				$"Unexpected method: {_concreteTypeName}.{_methodName} received for mdToken: {_mdToken} in module: {_resolutionModule?.FullyQualifiedName ?? "NULL"}, {_resolutionModule?.ModuleVersionId ?? _moduleVersionId}";
		...
		private void ValidateRequirements()
		{
			...
			{
				throw new ArgumentException(
					$"The length of {nameof(_namespaceAndNameFilter)} must match the length of {nameof(_parameters)} + 1 for the return type.");
			}
			...
			if (_explicitParameterTypes != null)
			{
				...
				for (var i = 0; i < _explicitParameterTypes.Length; i++)
				{
					...
					{
						throw new ArgumentException(
							$"Parameter Index {i}: Explicit type {explicitType.FullName} is not assignable from {parameterType}");
					}
				}
			}
		}
