...
namespace Elastic.Apm.Profiler.Managed.Integrations.Kafka
{
	...
	public class KafkaProduceSyncDeliveryHandlerIntegration
	{
		...
		public static CallTargetState OnMethodBegin<TTarget, TKey, TValue, TActionOfDeliveryReport>(TTarget instance, string topic, TKey key,
			TValue value, TActionOfDeliveryReport handler
		)
		{
			if (handler is null)
			{
