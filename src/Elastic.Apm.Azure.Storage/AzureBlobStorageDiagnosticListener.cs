...
namespace Elastic.Apm.Azure.Storage
{
	...
	public class AzureBlobStorageDiagnosticListener : DiagnosticListenerBase
	{
		...
		private void OnStart(KeyValuePair<string, object> kv, string action)
		{
			...
			var spanName = blobUrl.BlobName is null
				? $"{AzureBlobStorage.SpanName} {action} {blobUrl.ContainerName}"
				: $"{AzureBlobStorage.SpanName} {action} {blobUrl.ContainerName}/{blobUrl.BlobName}";
			...
			span.Context.Destination = new Destination
			{
				...
				Service = new Destination.DestinationService
				{
					...
					Resource = blobUrl.BlobName is null
						? $"{AzureBlobStorage.SubType}/{blobUrl.ContainerName}"
						: $"{AzureBlobStorage.SubType}/{blobUrl.ContainerName}/{blobUrl.BlobName}",
		...
		private class BlobUrl
		{
			public BlobUrl(string url)
			{
				...
				ContainerName = builder.Uri.Segments.Length > 1
					? builder.Uri.Segments[1].TrimEnd('/')
					: null;

				BlobName = builder.Uri.Segments.Length > 2
					? builder.Uri.Segments[2].TrimEnd('/')
					: null;
			}
