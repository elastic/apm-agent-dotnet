...
use crate::{
    cil::{
        uncompress_data, uncompress_token, CorExceptionFlag, Method,
        Operand::{
    ...
    match parts.len() {
        ...
        2 => {
            ...
            if let Ok(parent_type_def) = metadata_import.find_type_def_by_name(parts[0], None) {
                ...
                log::debug!(
                    "unable to load parent type_def={} for nested class: {}, module={}",
                    parts[0],
        ...
        _ => {
            log::warn!(
                "invalid type_def. only one layer of nested classes are supported: {}, module={}",
                target_method_type_name,
                assembly_name
            );
            return None;
...
pub fn flatten_integrations(
    integrations: Vec<Integration>,
    calltarget_enabled: bool,
) -> Vec<IntegrationMethod> {
    integrations
        .into_iter()
        .flat_map(|i| {
            let name = i.name.clone();
            i.method_replacements
                .into_iter()
