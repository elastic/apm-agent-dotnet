...
if ($? -ne 0) {
  Write-Host "visualstudio2019buildtools installation failed. See the logs."
  exit 1
}
...
if ($? -ne 0) {
  Write-Host "visualstudio2019professional installation failed. See the logs."
  exit 1
}
& choco install visualstudio2019-workload-netweb -m -y --no-progress --force -r --version=1.0.0
if ($? -ne 0) {
  Write-Host "visualstudio2019-workload-netweb installation failed. See the logs."
  exit 1
}
