                  ...
                  steps {
                    ...
                    dir("${BASE_DIR}"){
                      ...
                      dotnet test -v n -r target -d target/diag.log --logger:"xunit" --no-build \
                        ...
                        /p:Exclude='"[Elastic.Apm.Tests]*,[AspNetCoreSampleApp*]*,[xunit*]*"' \
                  ...
                  stage('Test') {
                    steps {
                      ...
                      dir("${BASE_DIR}"){
                        ...
                        bat label: 'Test & Coverage', script: 'dotnet test -v n -r target -d target\\diag.log --logger:xunit --no-build /p:CollectCoverage=true /p:CoverletOutputFormat=cobertura /p:CoverletOutput=target\\Coverage\\ /p:Exclude=\\"[Elastic.Apm.Tests]*,[AspNetCoreSampleApp*]*,[xunit*]*\\" /p:Threshold=0 /p:ThresholdType=branch /p:ThresholdStat=total'
