...
::::{note}
Setting this to `false` reduces memory allocations, network bandwidth, and disk space used by {{product.elasticsearch}}.
::::
...
::::{note}
All errors that are captured during a request to an ignored URL are still sent to the {{product.apm-server}} regardless of this setting.
::::
...
| Default | Type |
| --- | --- |
| `/VAADIN/*, /heartbeat*, /favicon.ico, *.js, *.css, *.jpg, *.jpeg, *.png, *.gif, *.webp, *.svg, *.woff, *.woff2, *.ttf, *.otf` | Comma separated string |
...
Set this property to true when requests made by the {{product.apm-agent-dotnet}} should, if requested by the server, be authenticated using the credentials of the currently logged on user.

This is useful when using windows authentication on a proxy, that routes {{product.apm-agent-dotnet}} requests to the {{product.apm-server}}.
...
## `{{product.apm-server}} proxy` [add-proxy-apm]

Add a proxy to the {{product.apm-server}}.
