...
| ASP.NET Core | built-in (.NET), via startup hook† |
| HTTP client | built-in (.NET), via startup hook† |
...
* **Host access**: you must be able to set environment variables for the process or service you want to instrument (for example, via a Dockerfile, service definition, or system settings).
* **Supported platform**: verify that your runtime and architecture appear in the support table above. If you are unsure which runtime your app targets, see the note above about .NET vs .NET Framework.
...
2. Replace `<your-base-image>` with your application's base image (e.g. `mcr.microsoft.com/dotnet/aspnet:8.0`).
