...
::::{note}
If you are using the [Elastic {{product.apm}} Profiler](/reference/setup-auto-instrumentation.md), Kafka is instrumented automatically via IL rewriting for `Confluent.Kafka` ≥1.4.0 <3.0.0 with no code changes required. The `Confluent.Kafka.Extensions.Diagnostics` package is not needed in that case.
::::
