...
For non-local deployments, configure at least two settings to connect the agent to your {{product.apm-server}}:
...
::::{note}
For local development against a default {{product.apm-server}} with no authentication, these defaults are sufficient and no configuration is required.
::::
...
{{product.kibana}} generates an API key by default when configuring the {{product.apm}} integration, making [`ApiKey`](/reference/config-reporter.md#config-api-key) the recommended authentication method. If your {{product.apm-server}} is configured with a secret token instead, use [`SecretToken`](/reference/config-reporter.md#config-secret-token).
...
::::{note}
If the agent cannot reach the {{product.apm-server}}, for example, due to a missing or incorrect URL or authentication setting, it continues to run but cannot send data. Check the agent logs if you are not seeing data in {{product.apm}}. See [`LogLevel`](/reference/config-supportability.md#config-log-level) to increase log verbosity.
::::
...
| [Reporter](/reference/config-reporter.md) | {{product.apm-server}} connection, authentication, and reporting intervals |
...
The .NET Agent supports [Central configuration](docs-content://solutions/observability/apm/apm-agent-central-configuration.md), which allows you to fine-tune certain configurations using {{product.kibana}}. This feature is enabled in the Agent by default, with [`CentralConfig` ([1.1])](/reference/config-core.md#config-central-config). The agent polls for configuration changes at the interval specified by the {{product.apm-server}}, defaulting to 5 minutes. Changes take effect after the next poll.
