...
This approach instruments the following
...
Using global environment variables causes the Elastic profiler auto instumentation to be loaded for **any** .NET process started on the
...
Environment variables can be added to specific service srun with systemd
...
environment variables have a global scope that would end up auto-instrumnenting
...
will try to write log files to `${ELASTIC_APM_PROFILER_HOME}/logs`.
