...
. Unzip the provided zip file into a folder on the host that is hosting the application to instrument.
...
and the https://docs.docker.com/storage/volumes/[unzipped directory can be mounted as a volume]. 
The example below mounts the unzipped directory to `/elastic-apm-dotnet-auto-instrumentation`
