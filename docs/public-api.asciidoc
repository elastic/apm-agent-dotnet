...
If you don't explicitly initialize the agent, it will be started with a default configuration. This means the agent will read <<configuration, configuration settings>> from environment variables. In case you don't set an environment variable, the Agent will use the default value (in case of `ServerUrls` this is `http://localhost:8200`).

This implicit initialization of the agent happens on the first call on the `Elastic.Apm.Agent` class.

NOTE: One exception is the `Elastic.Apm.Agent.IsConfigured` method. This method never initializes the agent, it only checks whether the agent is already initialized.

Another example for initialization is when you enable the agent with one of the technology specific methods from the <<setup>> instructions. Specifically when the `UseElasticApm` or `UseAllElasticApm` method is called in ASP.NET Core or when the IIS module is initialized in case of IIS applications.
...
In case you would like to replace one of the agent components, you can do so by calling the `Elastic.Apm.Agent.Setup(AgentComponents)` method.

In the AgentComponents you can pass following optional components to the agent:

- `IApmLogger`: A logger implementation which will be used to print log messages. Default: A console logger.
...
In case of ASP.NET Core, when you turn on the agent with the `UseAllElasticApm` method, the agent will do this automatically.

In case of a typical console application, you need to do this manually.
