...
[float]
[[server-use-windows-credentials]]
==== `ServerUseWindowsCredentials`

[options="header"]
|============
| Environment variable name | IConfiguration or Web.config key
| `ELASTIC_APM_SERVER_USE_WINDOWS_CREDENTIALS` | `ElasticApm:ServerUseWindowsCredentials`
|============
...
| <<config-server-use-windows-credentials,`ServerUseWindowsCredentials`>> | No | Reporter
