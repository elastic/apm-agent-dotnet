...
| <<config-transaction-max-spans,`TransactionMaxSpans`>>  | Yes | Performance, Core
| <<config-transaction-sample-rate,`TransactionSampleRate`>> | Yes | Performance, Core
|============

NOTE: Option being reloadable means that the setting can be changed after agent's start.
