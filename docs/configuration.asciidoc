...
The maximal amount of time (in seconds) events are held in the queue until there is enough to send a batch.
It's possible for a batch to contain less then <<config-max-batch-event-count,`MaxBatchEventCount`>> events
...
then agent doesn't hold events in the queue but sends them immediately.  
...
==== `MaxBatchEventCount` (added[1.0.2])
...
==== `MaxQueueEventCount` (added[1.0.2])
