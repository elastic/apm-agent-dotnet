...
For example in order to ignore URLs `/foo` and `/bar` you can set this configuration to `"/foo,/bar"`.
...
In other words, the matching always happens based on the request path, ignoring host or query string.
