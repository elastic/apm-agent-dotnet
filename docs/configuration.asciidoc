...
To use this type of setup, which is typical in an ASP.NET Core application, your applications `Startup.cs` file should contain code similar to the following:
...
In certain scenarios--like when you're not using ASP.NET Core, the {agent} won't be activated with the `UseElasticApm()` method.
In this case, set the agents log level with <<config-log-level,`ElasticApm:LogLevel`>>, as shown in the following `appsettings.json` file:
...
Once implemented, use the <<config-full-framework-configuration-reader-type, `FullFrameworkConfigurationReaderType`>> setting.
...
NOTE: The service name must conform to this regular expression: `^[a-zA-Z0-9 _-]+$`. Your service name must only contain characters from the ASCII alphabet, numbers, dashes, underscores, and spaces. Characters in service name that don't match regular expression will be replaced by `_` symbol. 
...
It's important to be consistent when naming environments across {agent}s.
...
This setting can be changed after the {agent} is started.
...
This setting can be changed after {agent} is started.
...
If you do find sensitive data in your Elasticsearch index, add an additional entry to this list.
...
These are labels that are added to all events with the format `key=value[,key=value[,...]]`.
...
A string used to ensure that only your {agent}s can send data to your APM server.

Both the {agent}s and the APM server have to be configured with the same secret token.
Use this setting if the APM Server requires a token (e.g. APM Server in Elastic Cloud).
...
A base64-encoded string used to ensure that only your {agent}s can send data to your APM server.
...
The path to a PEM-encoded certificate used for SSL/TLS by APM server, is used to perform validation 
...
This determines which content types should be captured.
...
When an incoming HTTP request is detected, the request path will be tested against each element in this list.
...
When this setting is `true`, the {agent} also adds the header `elasticapm-traceparent` for backwards compatibility with older versions of Elastic APM {agent}s. Versions prior to `1.3.0` only read the `elasticapm-traceparent` header.
...
A list of namespaces to exclude when reading an exceptions StackTrace to determine the culprit.
...
Setting this to `0` disables the stack trace collection. Any positive integer value will be used as the maximum number of frames to collect. Setting it to -1 means that all frames will be collected.
