...
[float]
===== Features
- {pull}1057[#1057] Introduce `GetLabel<T>` method on `IExecutionSegment` (issue: https://github.com/elastic/apm-agent-dotnet/issues/1033[#1033])

[float]
===== Bug fixes
- {pull}1052[#1052] Increased transaction duration due to stack trace capturing (issue: https://github.com/elastic/apm-agent-dotnet/issues/1039[#1039])
- {pull}1053[#1053] Warning with `Synchronous operations are disallowed` on ASP.NET Core during request body capturing (issue: https://github.com/elastic/apm-agent-dotnet/issues/1044[#1044])
- {pull}1042[#1042] SqlClient instrumentation on .NET 5 (issue: https://github.com/elastic/apm-agent-dotnet/issues/1025[#1025])
- {pull}1060[#1060] `UseAllElasticApm` with `IHostBuilder` missing auto instrumentation (issue: https://github.com/elastic/apm-agent-dotnet/issues/1059[#1059])
...
[float]
===== Features
- {pull}828[#828] Agent loading with zero code change on .NET Core (issue: https://github.com/elastic/apm-agent-dotnet/issues/71[#71])
...
- {pull}1002[#1002] `ITransaction.SetService` API to support multiple services in a single process (issue: https://github.com/elastic/apm-agent-dotnet/issues/1001[#1001])
- {pull}1003[#1003] Collecting cloud metadata (supporting AWS, Azure,  GCP) (issue: https://github.com/elastic/apm-agent-dotnet/issues/918[#918])
- {pull}973[#973] Transaction grouping on ASP.NET (issue: https://github.com/elastic/apm-agent-dotnet/issues/[#201])
- {pull}913[#913] Entity Framework 6 support on .NET Core (issue: https://github.com/elastic/apm-agent-dotnet/issues/902[#902])


[float]
===== Bug fixes
- {pull}992[#992] On ASP.NET Core `CurrentTransaction` is null in some cases (issues: https://github.com/elastic/apm-agent-dotnet/issues/934[#934], https://github.com/elastic/apm-agent-dotnet/issues/972[#972])
- {pull}971[#971] Avoid double initialization in `HostBuilderExtensions`
- {pull}999[#999] Capture body with large file error (issue: https://github.com/elastic/apm-agent-dotnet/issues/960[#960])

[float]
===== Breaking changes
- Binary compatibility on `IExecutionSegment.CaptureException` and `IExecutionSegment.CaptureError` with libraries depending on previous version. If this happens you need to update `Elastic.Apm` to 1.7.0 in your projects (Issue: (https://github.com/elastic/apm-agent-dotnet/issues/1067)[#1067])
